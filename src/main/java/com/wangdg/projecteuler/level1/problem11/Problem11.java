package com.wangdg.projecteuler.level1.problem11;
...
/**
 ...
 注意点：对角线有两个方向：左上->右下， 右上->左下（做的时候忽略了这个方向）
...
public class Problem11 {
    ...
    protected static long findMaxProduct(int[][] array) {
        ...
        for (int i = 0; i <= 16; i++) {
            for (int k = 0; k <= 16; k++) {
                long val = array[i][k] * array[i + 1][k + 1] * array[i + 2][k + 2] * array[i + 3][k + 3];
                if (val > product) {
                    product = val;
                }
            }
        ...
        for (int i = 0; i <= 19; i++) {
            for (int k = 0; k <= 16; k++) {
                long val = array[i][k] * array[i][k + 1] * array[i][k + 2] * array[i][k + 3];
                if (val > product) {
                    product = val;
                }
            }
        ...
        for (int i = 0; i <= 16; i++) {
            for (int k = 0; k <= 19; k++) {
                long val = array[i][k] * array[i + 1][k] * array[i + 2][k] * array[i + 3][k];
                if (val > product) {
                    product = val;
                }
            }
